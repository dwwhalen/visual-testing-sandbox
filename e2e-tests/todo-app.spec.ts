...
test.describe('Item', () => {
  ...
  test('should allow me to edit an item', async ({ page }) => {
    ...
    const textInput = secondTodo.getByTestId('text-input');
    await expect(textInput).toHaveValue(TODO_ITEMS[1]);
    await textInput.fill('buy some sausages');
...
test.describe('Editing', () => {
  ...
  test('should cancel edits on tab', async ({ page }) => {
    const todoItems = page.getByTestId('todo-item');
    const secondTodo = todoItems.nth(1);
    await secondTodo.dblclick();
    const textInput = secondTodo.getByTestId('text-input');
...
test.describe('Clear completed button', () => {
  ...
  test('should be visible even when there are no items that are completed', async ({ page }) => {
    await page.locator('.todo-list li .toggle').first().check();
    await page.getByRole('button', { name: 'Clear completed' }).click();
    await expect(page.getByRole('button', { name: 'Clear completed' })).toBeVisible();
  });
